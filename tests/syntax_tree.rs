...
//! losslessness, that every node's text really is the input at its range, that node and
//! token kinds never mix, and that no input — truncated, malformed or random — panics or
//! hangs the parser.
