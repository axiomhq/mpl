...
// ── moved from the former `src/parser/tests.rs` (compile-driven; the pest
//    `Rule`-internal tests there were removed with the pest grammar) ──

#[test]
fn compute_query_post_compute_aggregates() -> Result<(), Box<dyn std::error::Error>> {
    let query = "
    (
        test:metric_a[30m..]
