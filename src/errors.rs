...
use crate::query::{ParamDeclaration, ParamType};

/// A grammar construct that the parser recognises syntactically but the
/// backend does not (yet) support. Replaces the former `pest`-`Rule`-typed
/// `NotSupported` payload now that the grammar is hand-written in
/// [`crate::wparser`].
...
/// Error returned when parsing an externally-provided `param` value
/// (`ProvidedParams::parse_and_validate`). Lives here (pest-free) now that the
/// grammar is hand-written in [`crate::wparser`].
#[derive(Debug, thiserror::Error)]
pub enum ParseParamError {
    /// The value could not be parsed at all.
    #[error("Failed to parse: {0}")]
    Parse(#[from] ParseError),
    /// The value was not a valid bool.
    #[error("Failed to param as bool: {0}")]
    ParseBool(<bool as FromStr>::Err),
    /// The value was not a valid float.
    #[error("Failed to parse as float: {0}")]
    ParseFloat(#[from] ParseFloatError),
    /// The value could not be turned into a `SharedString`.
    ...
    #[error(
        "Param is declared as type {declared_typ}, but the provided value was not a valid {found}"
    )]
    TypeMismatch {
        /// The declared param type.
        declared_typ: ParamType,
        /// A short description of the expected value shape.
        found: &'static str,
    },
    /// `Null`-typed params cannot be provided.
